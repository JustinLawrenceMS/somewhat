      ...
      try {
        ...
        if (typeof value === "object" && value !== null) {
          ...
          if (
            ...
            (typeof value === "string" &&
              this.matchWithWildcard(value, searchTerm))
          ) {
  ...
  searchDOM(element, searchTerm = this.pattern, path = "") {
    ...
    if (element.attributes) {
      for (const attr of element.attributes) {
        ...
        if (
          this.matchWithWildcard(attr.name, searchTerm) ||
          this.matchWithWildcard(attr.value, searchTerm)
        ) {
          results.push({ path: attrPath, value: attr.value });
        }
      }
    }
